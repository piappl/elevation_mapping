...
public:
   ...
   * @return True if the PostprocessorPool accepted the task. If false the PostprocessorPool had no available threads
   * and discarded the task.
   */
...
private:
  ...
  //! Container holding the service ids which have corresponding threads. The only object that is used in a mutual
  //! exclusive manner and must be protected by availableServicesMutex_.
