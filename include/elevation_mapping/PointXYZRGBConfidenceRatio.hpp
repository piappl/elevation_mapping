...
{                   // NOLINT(cppcoreguidelines-pro-type-union-access)
  PCL_ADD_POINT4D;  // NOLINT(cppcoreguidelines-pro-type-union-access, readability-const-return-type,
                    // modernize-avoid-c-arrays) This adds the members x,y,z which can also be accessed using the point
                    // (which is float[4])
...
POINT_CLOUD_REGISTER_POINT_STRUCT(pcl::_PointXYZRGBConfidenceRatio,  // NOLINT(modernize-avoid-c-arrays,
                                                                     // readability-const-return-type) here we assume a
                                                                     // XYZ + "confidence_ratio" (as fields)
                                  (float, x, x)                                 // NOLINT
                                  (float, y, y)                                 // NOLINT
                                  (float, z, z)                                 // NOLINT
