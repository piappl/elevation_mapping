...
/**
 * @brief An Input feeds data to ElevationMapping callbacks. E.g it holds a subscription to an sensor source and
 * registered an appropriate ElevationMapping callback.
 */
class Input
{
public:
   ...
   */
  bool configure(std::string& inputSourceName, const std::string& sourceConfigurationName,
...
private:
   ...
   * @param generalSensorProcessorParameters  General parameters needed for the sensor processor that are not specific
   * to this sensor processor.
...
void Input::registerCallback(ElevationMapping& map, CallbackT<MsgT> callback)
{
  ...
  std::function<void(sensor_msgs::msg::PointCloud2::ConstSharedPtr msg)> bound_callback_func = std::bind(
      callback, std::ref(map), std::placeholders::_1, parameters.publishOnUpdate_, std::ref(sensorProcessor_));
  ...
  RCLCPP_INFO(nodeHandle_->get_logger(), "Subscribing to %s: %s, queue_size: %i.", parameters.type_.c_str(),
              parameters.topic_.c_str(), parameters.queueSize_);
}
