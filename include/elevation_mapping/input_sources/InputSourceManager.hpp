...
template <typename... MsgT>
bool InputSourceManager::registerCallbacks(ElevationMapping& map,
                                           std::pair<const char*, Input::CallbackT<MsgT>>... callbacks)
{
  ...
  for (Input& source : sources_)
  {
    ...
    {
      RCLCPP_WARN(nodeHandle_->get_logger(), "The configuration contains input sources of an unknown type: %s",
