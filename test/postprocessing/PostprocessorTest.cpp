...
/**
 * We read in a postprocessing (mock) configuration that takes 150ms to execute. We test whether the postprocessorPool
 * accepts/discards the tasks for various configurations of time between tasks and number of threads in the pool.
 */
...
public:
  static void checkAcceptedTasks(uint poolSize, uint timeBetweenConsecutiveTasks,
                                 std::vector<bool> expectedAcceptanceOutcomes)
  {
    ...
    for (auto expectedOutcome : expectedAcceptanceOutcomes)
    {
      ...
      {
        ASSERT_FALSE(accepted) << "Postprocessor pool accepted task number: " << taskNumber
...
TEST_F(RosFixture, FiveTasksOneThreadSimultaneously)
{  // NOLINT
  checkAcceptedTasks(1, 0, { true, false, false, false, false });
}

TEST_F(RosFixture, FiveTasksTwoThreadSimultaneously)
{  // NOLINT
  checkAcceptedTasks(2, 0, { true, true, false, false, false });
}

TEST_F(RosFixture, EnoughTimeToProcess)
{  // NOLINT
  checkAcceptedTasks(1, 200, { true, true, true, true, true });
}

TEST_F(RosFixture, EnoughTimeToProcessWithTwoThreads)
{  // NOLINT
  checkAcceptedTasks(2, 100, { true, true, true, true, true });
}

TEST_F(RosFixture, ProcessEverySecond)
{  // NOLINT
  checkAcceptedTasks(1, 100, { true, false, true, false, true });
}

TEST_F(RosFixture, TwoThreadsWithMiss)
{  // NOLINT
