...
{  // NOLINT
  ...
  bool success = inputSourceManager.registerCallbacks(
      map, make_pair("pointcloud", &elevation_mapping::ElevationMapping::pointCloudCallback));
  ASSERT_FALSE(success);
}

TEST(ElevationMap, Constructor)
{  // NOLINT
  auto nodeHandle = rclcpp::Node::make_shared("_");
  elevation_mapping::ElevationMapping map(nodeHandle);
}

TEST(InputSources, ListeningToTopicsAfterRegistration)
{  // NOLINT
  ...
  {
  public:
    ElevationMappingWithInputSourcesAccessor(std::shared_ptr<rclcpp::Node> nodeHandle)
    ...
    {
      return inputSources_.getNumberOfSources();
    }
  } map{ nodeHandle };
