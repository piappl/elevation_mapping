...
{
  if (config.size() == 0)
  {  // Use Empty array as special case to explicitly configure no inputs.
    return true;
  }
  ...
  for (auto inputConfig : config)
  {
    ...
    // return leading / -> rclcpp::expand_topic_or_service_name(sourceConfigurationName + "/" + inputConfig,
    // nodeHandle_->get_name(), nodeHandle_->get_namespace() auto subnode =
    // nodeHandle_->create_sub_node(sourceConfigurationName + "/" + inputConfig);
