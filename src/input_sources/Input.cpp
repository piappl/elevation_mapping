...
bool Input::configure(std::string& inputSourceName, const std::string& sourceConfigurationName,
                      const SensorProcessorBase::GeneralParameters& generalSensorProcessorParameters)
{
  ...
  {
    RCLCPP_ERROR(nodeHandle_->get_logger(), "Could not configure input source %s because no type was given.",
                 inputSourceName.c_str());
  }

  if (!nodeHandle_->get_parameter(inputSourceName + ".topic", parameters.topic_))
  {
    RCLCPP_ERROR(nodeHandle_->get_logger(), "Could not configure input source %s because no topic was given.",
                 inputSourceName.c_str());
  }

  if (!nodeHandle_->get_parameter(inputSourceName + ".queue_size", parameters.queueSize_))
  {
    RCLCPP_ERROR(nodeHandle_->get_logger(), "Could not configure input source %s because no queue_size was given.",
                 inputSourceName.c_str());
  }
  ...
  if (!nodeHandle_->get_parameter(inputSourceName + ".publish_on_update", parameters.publishOnUpdate_))
  {
    RCLCPP_ERROR(nodeHandle_->get_logger(),
                 "Could not configure input source %s because no publish_on_update was given.",
                 inputSourceName.c_str());
  }
  ...
  {
    RCLCPP_ERROR(nodeHandle_->get_logger(),
                 "Could not configure input source %s because no sensor_processor was given.", inputSourceName.c_str());
  }
  ...
  RCLCPP_INFO(
      nodeHandle_->get_logger(), "Configured %s:%s @ %s (publishing_on_update: %s), using %s to process data.\n",
      parameters.type_.c_str(), parameters.name_.c_str(),
      rclcpp::expand_topic_or_service_name(parameters.topic_, nodeHandle_->get_name(), nodeHandle_->get_namespace())
          .c_str(),
      parameters.publishOnUpdate_ ? "true" : "false", sensorProcessorType.c_str());
  return true;
}
