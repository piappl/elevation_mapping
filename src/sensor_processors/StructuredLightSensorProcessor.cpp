 ...
 * sensorModelNormalFactorC_)^2; standardDeviationInLateralDirection = sensorModelLateralFactor_ * measurementDistance
 * Taken from: Nguyen, C. V., Izadi, S., & Lovell, D., Modeling Kinect Sensor Noise for Improved 3D Reconstruction and
 * Tracking, 2012.
 */

StructuredLightSensorProcessor::StructuredLightSensorProcessor(
    std::shared_ptr<rclcpp::Node>& nodeHandle, const SensorProcessorBase::GeneralParameters& generalParameters)
...
bool StructuredLightSensorProcessor::readParameters(std::string& inputSourceName)
{
  ...
  nodeHandle_->declare_parameter(inputSourceName + ".sensor_processor.cutoff_min_depth",
                                 rclcpp::ParameterValue(std::numeric_limits<double>::min()));
  nodeHandle_->declare_parameter(inputSourceName + ".sensor_processor.cutoff_max_depth",
                                 rclcpp::ParameterValue(std::numeric_limits<double>::max()));

  nodeHandle_->get_parameter(inputSourceName + ".sensor_processor.normal_factor_a", sensorParameters_["normal_factor_"
                                                                                                      "a"]);
  nodeHandle_->get_parameter(inputSourceName + ".sensor_processor.normal_factor_b", sensorParameters_["normal_factor_"
                                                                                                      "b"]);
  nodeHandle_->get_parameter(inputSourceName + ".sensor_processor.normal_factor_c", sensorParameters_["normal_factor_"
                                                                                                      "c"]);
  nodeHandle_->get_parameter(inputSourceName + ".sensor_processor.normal_factor_d", sensorParameters_["normal_factor_"
                                                                                                      "d"]);
  nodeHandle_->get_parameter(inputSourceName + ".sensor_processor.normal_factor_e", sensorParameters_["normal_factor_"
                                                                                                      "e"]);
  nodeHandle_->get_parameter(inputSourceName + ".sensor_processor.lateral_factor", sensorParameters_["lateral_factor"]);
  nodeHandle_->get_parameter(inputSourceName + ".sensor_processor.cutoff_min_depth", sensorParameters_["cutoff_min_"
                                                                                                       "depth"]);
  nodeHandle_->get_parameter(inputSourceName + ".sensor_processor.cutoff_max_depth", sensorParameters_["cutoff_max_"
                                                                                                       "depth"]);
  ...
  // nodeHandle_.param("sensor_processor/cutoff_min_depth", sensorParameters_["cutoff_min_depth"],
  // std::numeric_limits<double>::min()); nodeHandle_.param("sensor_processor/cutoff_max_depth",
                                                      ...
                                                      Eigen::VectorXf& variances)
{
  ...
  for (unsigned int i = 0; i < pointCloud->size(); ++i)
  {
    ...
    heightVariance += static_cast<float>(sensorJacobian * sensorVariance * sensorJacobian.transpose()) /
