...
StereoSensorProcessor::StereoSensorProcessor(std::shared_ptr<rclcpp::Node>& nodeHandle,
...
bool StereoSensorProcessor::readParameters(std::string& inputSourceName)
{
  ...
  nodeHandle_->declare_parameter(inputSourceName + ".sensor_processor.depth_to_disparity_factor",
  ...
  nodeHandle_->get_parameter(inputSourceName + ".sensor_processor.depth_to_disparity_factor", sensorParameters_["depth_"
                                                                                                                "to_"
                                                                                                                "dispar"
                                                                                                                "ity_"
                                                                                                                "facto"
  ...
  // nodeHandle_.param("sensor_processor/depth_to_disparity_factor", sensorParameters_["depth_to_disparity_factor"],
  // 0.0); nodeHandle_.param("sensor_processor/cutoff_min_depth", sensorParameters_["cutoff_min_depth"],
                                             ...
                                             Eigen::VectorXf& variances)
{
  ...
  // Preparations for#include <pcl/common/transforms.h> robot rotation Jacobian (J_q) to minimize computation for every
  ...
  for (unsigned int i = 0; i < pointCloud->size(); ++i)
  {
    ...
    double disparity =
        sensorParameters_.at("depth_to_disparity_factor") / point.z;  // NOLINT(cppcoreguidelines-pro-type-union-access)
